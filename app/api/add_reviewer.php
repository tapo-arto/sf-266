...
/**
 * Add Reviewer API Endpoint
 * 
 * Adds a new reviewer to a flash without removing existing reviewers.
 * Sends email notification to the new reviewer.
 * 
 * Parameters:
 * - flash_id: The ID of the flash (required)
 * - user_id: The ID of the user to add as reviewer (required)
 */
...
try {
    ...
    if ($checkStmt->fetch()) {
        ...
        echo json_encode(['ok' => false, 'error' => 'User is already a reviewer for this flash']);
        exit;
    }
    ...
    try {
        ...
        $subjects = [
            'fi' => 'Sinut on lisätty SafetyFlash-tarkistajaksi',
            'sv' => 'Du har lagts till som SafetyFlash-granskare',
            'en' => 'You have been added as SafetyFlash reviewer',
            'it' => 'Sei stato aggiunto come revisore SafetyFlash',
            'el' => 'Προστεθήκατε ως ελεγκτής SafetyFlash'
        ];
        
        $bodies = [
            'fi' => "Hei {$userName},\n\nSinut on lisätty tarkistajaksi SafetyFlashille: {$flashTitle}\n\nVoit tarkastella SafetyFlashia järjestelmässä.",
            'sv' => "Hej {$userName},\n\nDu har lagts till som granskare för SafetyFlash: {$flashTitle}\n\nDu kan granska SafetyFlash i systemet.",
            'en' => "Hello {$userName},\n\nYou have been added as a reviewer for SafetyFlash: {$flashTitle}\n\nYou can review the SafetyFlash in the system.",
            'it' => "Ciao {$userName},\n\nSei stato aggiunto come revisore per SafetyFlash: {$flashTitle}\n\nPuoi rivedere il SafetyFlash nel sistema.",
            'el' => "Γεια σου {$userName},\n\nΠροστεθήκατε ως ελεγκτής για το SafetyFlash: {$flashTitle}\n\nΜπορείτε να ελέγξετε το SafetyFlash στο σύστημα."
        ];
    ...
    error_log('add_reviewer error: ' . $e->getMessage());
